...
4. STATE CHANGES — UPDATE TO THE CURRENT STATE: when a fact changes the state of something — "sold X", "X died", "moved to Y", a revised count or price — UPDATE the matching observation so its text reads as the present state, carrying dates where they matter. Stay within that one facet and keep it concise: never pull in detail from other observations. State a discrete change once and plainly — "User owned a dog named Rex who died on March 15, 2025". For a fluctuating quantity — a count, price, percentage, weight, balance, allocation — keep only the latest value and its as-of date; each new reading SUPERSEDES the prior one, so never accumulate a dated time-series of every past value. Carry a single earlier point only when the trend itself is the fact — "35.6% as of May 30, up from ~5% in early May".
...
7. PRESERVE HISTORY, NOT STALE VALUES: a significant discrete event — sold, died, moved, changed — is history; preserve it. Never DELETE the observation that records one; delete only when an observation is restated identically or is truly meaningless, and be very conservative about it. But preserving history means protecting the event, not hoarding every value that preceded the current one — a superseded reading of a fluctuating quantity is not history, it is a stale snapshot, and rule 4 replaces it. Record the event once and concisely, never as a running play-by-play of every revision that led to where things stand now.
...
10. BOUNDED LENGTH; UPDATE REWRITES, NEVER APPENDS: every observation — whether freshly CREATED or UPDATED — has a soft ceiling of about 4-5 sentences, roughly 80 words: a budget to stay under, not a target to fill. On CREATE, if a single source fact is long or verbose, COMPRESS it to its essential current state within the budget — do not copy the fact verbatim. On UPDATE, produce a fresh, self-contained statement that REPLACES the prior text rather than bolting new clauses onto it; when merging new evidence would push it past the ceiling, COMPRESS rather than grow — state the current resolved state, keep at most the single most significant prior transition, and drop intermediate detail. A decision or stance that evolved or reversed is recorded as its FINAL resolved state plus at most a one-clause note of what it superseded — never the full step-by-step sequence of every revision. Adding source_fact_ids as evidence (rule 1) does NOT require adding text: an observation can accumulate many supporting facts while its text stays bounded."""
