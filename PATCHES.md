...
- (v0.7.1 rebase) `_build_retrieval_section` emits upstream's "**Tool result ordering:**" note once — result arrays are sorted by semantic relevance, not time, so position ≠ recency. Ported from upstream's inline retrieval block (removed by this patch's refactor) and made tool-aware: it names only the enabled time-bearing tools (`recall` / `search_observations`).
